...
public class MQClientAPIImpl {
    ...
    /**
     * 实例化一个mqclient api实现对象
     * @param nettyClientConfig nettyclient配置对象
     * @param clientRemotingProcessor netty远程事件处理器
     * @param rpcHook
     * @param clientConfig mqclient配置对象
     */
    public MQClientAPIImpl(final NettyClientConfig nettyClientConfig,
        ...
        //给netty client的处理器列表添加一个检查请求码状态的处理器
        this.remotingClient.registerProcessor(RequestCode.CHECK_TRANSACTION_STATE, this.clientRemotingProcessor, null);

        //给netty client的处理器列表添加一个通知已更改的消费者id的处理器
        this.remotingClient.registerProcessor(RequestCode.NOTIFY_CONSUMER_IDS_CHANGED, this.clientRemotingProcessor, null);

        //给netty client的处理器列表添加一个重置消息者客户端偏移的处理器
        this.remotingClient.registerProcessor(RequestCode.RESET_CONSUMER_CLIENT_OFFSET, this.clientRemotingProcessor, null);

        //给netty client的处理列表添加一个从客户端获取消费者状态的处理器
        this.remotingClient.registerProcessor(RequestCode.GET_CONSUMER_STATUS_FROM_CLIENT, this.clientRemotingProcessor, null);

        //给netty client的处理器列表添加一个获取消费者运行状态的处理器
        this.remotingClient.registerProcessor(RequestCode.GET_CONSUMER_RUNNING_INFO, this.clientRemotingProcessor, null);

        //给netty client的处理器列表添加一个直接消费信息的处理器
    ...
    public void updateNameServerAddressList(final String addrs) {
        ...
        for (String addr : addrArray) {//遍历多个地址
            //将地址添加到地址列表
            lst.add(addr);
        }
        //更新注册中心地址列表
