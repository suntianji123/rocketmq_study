...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    /**
     * 实例化一个默认生产者实现对象
     * @param defaultMQProducer 包装生产者实现的对象
     * @param rpcHook 钓
     */
    ...
    /**
     * 启动生产者服务
     * @param startFactory 是否mqclient对象
     * @throws MQClientException
     */
    public void  start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {//服务的状态
            //只是创建
            case CREATE_JUST:
                //设置服务状态为启动失败
                ...
                //如果组名不是CLIENT_INNER_PRODUCER_GROUP 更改实例名
                if (!this.defaultMQProducer.getProducerGroup().equals(MixAll.CLIENT_INNER_PRODUCER_GROUP)) {
                    this.defaultMQProducer.changeInstanceNameToPID();
                }

                //根据clientid创建并获取mqclient对象
                this.mQClientFactory = MQClientManager.getInstance().getAndCreateMQClientInstance(this.defaultMQProducer, rpcHook);

                //向mqclient所维护的消费者列表中注册一个生产者
                boolean registerOK = mQClientFactory.registerProducer(this.defaultMQProducer.getProducerGroup(), this);
                if (!registerOK) {//没有注册成功 设置 生产者服务的启动状态为只是创建
                ...
                //向生产者的主题发布信息表中添加一个生产者创建的主题对应的发布消息对象
                this.topicPublishInfoTable.put(this.defaultMQProducer.getCreateTopicKey(), new TopicPublishInfo());

                if (startFactory) {//如果启动mqclient对象
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {//发布信息为空 或者发布信息不是ok
            //向主题发布信息列表中放入一个新的发布信息对象
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            //调用mqclient 从注册中心服务器更新主题的路径信息
